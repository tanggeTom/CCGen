...
def read_examples(filename):
    ...
    examples=[]
    with open(os.path.join('./data/dataseta', filename + '_story.txt'), 'r', encoding='utf-8') as f:
        with open(os.path.join('./data/dataseta', filename + '_summ.txt'), 'r', encoding='utf-8') as f2:
            story_lines = f.readlines()
            summ_lines = f2.readlines()
            size = len(story_lines)
            for i in range(size):
                story = story_lines[i].strip()
                message = summ_lines[i].strip()
                examples.append(
                    Example(
                            idx = i,
                            source=story,
    ...
    return examples
    ######用例#####
    # with open(filename,encoding="utf-8") as f, open(filename, encoding="utf-8") as f2:
